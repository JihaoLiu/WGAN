...
class Reader():
  def __init__(self, tfrecords_file, image_size_w=128, image_size_h=128,
  ...
  def _preprocess(self, image):
    image = tf.image.resize_images(image, size=(self.image_size_w, self.image_size_h))
    image = utils.convert2float(image)
    image.set_shape([self.image_size_w, self.image_size_h, 3])
    return image

def test_reader():
  TRAIN_FILE_1 = 'data/tfrecords/plate_real.tfrecords'
  TRAIN_FILE_2 = 'data/tfrecords/plate_synthetic.tfrecords'
